...
# Author:       Justin Perry
#
# Purpose:      Phonebook Application for storing . Demonstrating OOP, Tkinter GUI module,
#               using Tkinter Parent and Child relationships.
#
# OS:  This code was written and tested to work with Windows 10.
...
# this will enter dummy data into table if there are no entried in the table so far...
def first_run(self):
    conn = sqlite3.connect('db_phonebook.db')
    with conn:
        cur = conn.cursor()
...
# this will count the number of records to return the value to the first run function
...
def onSelect(self,event):
    ...
    with conn:
        ...
        # the fetchall() method is a sqlite3 built in method to retreive the data from the query as a tuple (all the data)
        varBody = cursor.fetchall()
        # This returns a tuple and we can slice it into 4 parts using data[] during the iteration
        # this will delete any existing data in the text box (.delete) and insert the new data (.insert)
        for data in varBody:
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            # fetchone()[index#] fetches only one value, the specified index in between the brackets
